...
Linux AppImage builds generated by the official GitHub Actions workflow are cryptographically attested using GitHub Artifact Attestations, backed by Sigstore. The attestation links the AppImage to this repository, the exact source commit, and the workflow that produced it.

With the GitHub CLI installed, a downloaded AppImage can be verified with:

```bash
gh attestation verify opus2txt-1.0.0-linux-x86_64.AppImage -R SebRoLENS/opus2txt
```

Windows and macOS packages are currently not code-signed, so Windows SmartScreen or macOS Gatekeeper may display a security warning when opening them for the first time.
